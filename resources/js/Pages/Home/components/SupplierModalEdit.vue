...
<template>
    <Modal :modal-open="modalOpen" @update:modalOpen="emit('update:modalOpen', $event)"
           :title="supplier.external ? 'Visualizar Fornecedor' :'Atualizar Fornecedor'">
        <template #body>
            <form id="form" class="flex flex-col gap-4" @submit.prevent="updateSupplier">
                <FormSupplier :form="form"/>
            </form>
        </template>
        <template #footer v-if="!supplier.external">
            <button type="submit"
                    form="form"
